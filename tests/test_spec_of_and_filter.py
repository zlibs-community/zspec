...
class TestSpecOf:
    ...
    def test_of_composable(self) -> None:
        even = Specification[int].of(lambda x: x % 2 == 0)
        positive = Specification[int].of(lambda x: x > 0)
        spec = even & positive
        assert spec(4)
        assert not spec(-2)
    ...
    def test_of_with_named_function(self) -> None:
        ...
        spec = Specification[int].of(is_positive)
        assert spec(5)
    ...
    def test_of_without_name(self) -> None:
        ...
        spec = Specification[int].of(partial(gt, 5))
        assert spec(10)
...
class TestTrueFalse:
    def test_true(self) -> None:
        assert Specification[object].true()(None)
        assert str(Specification[object].true()) == "TRUE"

    def test_false(self) -> None:
        assert not Specification[object].false()(None)

    def test_true_composable(self) -> None:
        even = Specification[int].of(lambda x: x % 2 == 0)
        spec = Specification[int].true() & even
        assert spec(4)
        assert not spec(5)

    def test_false_composable(self) -> None:
        even = Specification[int].of(lambda x: x % 2 == 0)
        spec = Specification[int].false() | even
        assert spec(4)
        assert not spec(5)

    def test_true_xor_true(self) -> None:
        spec = Specification[object].true() ^ Specification[object].true()
